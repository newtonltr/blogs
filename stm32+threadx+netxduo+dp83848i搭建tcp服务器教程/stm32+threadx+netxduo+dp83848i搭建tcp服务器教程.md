# stm32+threadx+netxduo+dp83848i搭建tcp服务器教程
---
### 写在前面
...
region 0 的配置：
![alt text](region0-1.png)
region 1 的配置：
![alt text](region0-1-1.png)
...
![alt text](image-1.png)
 - 单独配置复位引脚
 - ![alt text](image.png)
 - 系统时钟时基使用TIM6，其他也可以，用hal库别用systick，时钟图如下：![alt text](image-2.png)
 - 取消生成systick和pend sv中断处理函数，因为后面要使用threadx来接管这些中断。![alt text](image-3.png)
到这里已经可以点击生成代码了。

# 三、Threadx, NetXDuo移植
软件包可在文末我上传到github的完整工程中获取。以下讲解移植的几个重要过程。

 1. Threadx或者NetXDuo都有port文件夹，其中分为GNU和AC5两个文件夹，用cmake方案gcc编译的话保留GNU文件夹，用keil mdk保留ac5文件夹![alt text](image-4.png)
...
# 六、测试网络
 ...
 1. 测试能否ping通![alt text](image-5.png)

 2. 测试转发功能![alt text](image-6.png)
